...
urlpatterns = [
    ...
    re_path(r'^logout/$', user.logout_view, name='logout'),
    re_path(r'^login/$', auth_views.LoginView.as_view(template_name='registration/login.html')),
    path('password_change/', auth_views.PasswordChangeView.as_view(template_name='user/change_pass.html')),
    path('password_change/done/', auth_views.PasswordChangeDoneView.as_view(template_name='user/change_pass_success.html')),
    path('password_reset/', auth_views.PasswordResetView.as_view(template_name='registration/password-reset.html')),
    path('password_reset/done/', auth_views.PasswordResetDoneView.as_view(template_name='registration/password-reset-done.html')),
